...
<div id="auth-modal" class="fixed inset-0 bg-gray-900 bg-opacity-80 backdrop-blur-sm flex items-center justify-center z-[60] hidden transition-opacity">
    <div class="bg-white rounded-2xl shadow-2xl p-8 max-w-sm w-full text-center border border-gray-100">
        ...
        <a href="login.php" class="block w-full bg-[#0A112F] text-white font-bold py-3 px-4 rounded-xl hover:bg-opacity-90 transition">Se connecter</a>
        <a href="index.php" class="block mt-4 text-sm text-gray-400 hover:text-gray-600">Retour à l'accueil</a>
    </div>
</div>
...
<div id="camp-content" class="hidden min-h-screen bg-gray-50 pb-20 font-sans">
    ...
    <div id="hero-section" class="relative min-h-[550px] w-full overflow-hidden group flex items-end">
        ...
        <div class="absolute top-10 left-10 z-20">
            <a href="index.php" class="flex items-center gap-2 px-5 py-2.5 rounded-full bg-white/10 backdrop-blur-md border border-white/20 text-white hover:bg-white/20 transition shadow-lg text-sm font-medium">
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    if (!campId) {
        loader.innerHTML = '<div class="text-center p-10"><h1 class="text-2xl font-bold text-red-500">Séjour introuvable</h1><a href="index.php" class="text-[#0A112F] underline mt-4 block">Retour accueil</a></div>';
        return;
    }
    ...
    try {
        ...
        if(camp.inscription_en_ligne == 1 && places > 0) {
            ctaHtml = `
                <a href="inscription.php?camp_t=${camp.token}" class="flex items-center justify-center gap-2 w-full bg-[#0A112F] hover:bg-opacity-90 text-white font-bold py-4 px-6 rounded-xl shadow-lg transform transition hover:scale-[1.02] text-center text-lg">
            ...
            ctaHtml = `<a href="${camp.lien_externe}" target="_blank" class="block w-full bg-[#0A112F] hover:bg-opacity-90 text-white font-bold py-3 px-6 rounded-xl text-center shadow transition">Voir le site officiel 🌐</a>`;
        } else if (places === 0) {
        ...
        console.error(e);
        loader.innerHTML = `<div class="p-8 text-center"><p class="text-red-600 font-bold text-lg mb-2">Erreur technique</p><p class="text-gray-600 text-sm bg-gray-100 p-4 rounded">${e.message}</p><a href="index.php" class="text-[#0A112F] underline mt-4 block">Retour accueil</a></div>`;
    }
});
...
async function contactOrganisateur(targetUserId, campId) {
    ...
    try {
        ...
        if(data.conversationId) window.location.href = `messagerie.php?conv_id=${data.conversationId}`;
