...
// Le fichier api/config.php est requis pour STRIPE_PUBLISHABLE_KEY
...
<main class="container mx-auto max-w-4xl px-4 py-12">
    ...
    <div id="inscription-content" class="hidden">
        ...
        <form id="form-inscription" onsubmit="handlePaymentSubmission(event)">
            <input type="hidden" name="camp_token" value="<?= htmlspecialchars($token) ?>">
            ...
            <div id="step-1" class="bg-white p-6 rounded-xl shadow-lg border mb-6 transition-all">
                ...
                <div class="mt-6 text-right">
                    <button type="button" id="btn-goto-2" class="bg-gray-300 text-white font-bold py-3 px-6 rounded-lg shadow disabled:bg-gray-300 disabled:cursor-not-allowed" disabled>
                        Continuer &rarr;
                    </button>
                </div>
            </div>
            ...
            <div id="step-2" class="hidden bg-white p-6 rounded-xl shadow-lg border mb-6 transition-all">
                ...
                <div class="mt-8 flex justify-between items-center">
                    <button type="button" id="btn-back-1" class="text-gray-500 hover:text-gray-800 underline">Retour</button>
                    <button type="button" id="btn-goto-3" class="bg-gray-400 text-white font-bold py-3 px-6 rounded-lg shadow cursor-not-allowed" disabled>
            ...
            <div id="step-3" class="hidden bg-white p-6 rounded-xl shadow-lg border mb-6 transition-all">
                ...
                <div class="bg-gray-50 p-6 rounded-xl border border-gray-200 text-center">
                    ...
                    <p class="text-xs text-gray-500 mb-6">Paiement sécurisé par Carte Bancaire</p>
                    
                    <button type="submit" id="btn-pay" class="w-full md:w-1/2 bg-green-600 hover:bg-green-700 text-white font-bold py-4 px-6 rounded-lg shadow-lg transform transition hover:scale-[1.02] disabled:opacity-50 disabled:cursor-not-allowed" disabled>
                        Confirmer et Payer
                    </button>
                    
                    <p id="payment-error-message" class="text-red-600 mt-3 font-medium hidden">Une erreur est survenue lors du paiement. Veuillez réessayer.</p>
                </div>
                <div class="mt-4 text-left">
                    <button type="button" id="btn-back-2" class="text-gray-500 hover:text-gray-800 underline">Retour</button>
                </div>
            </div>
        </form>
...
<script>
    // La clé publique est définie dans api/config.php et injectée ici
    ...
    document.addEventListener('DOMContentLoaded', async function() {
        ...
        let selectedTarifs = {}; // { childId: {tarifId, price, basePrice} }
        ...
        function renderPricing() {
            ...
            selectedChildrenIds.forEach((id, index) => {
                ...
                globalData.tarifs.forEach(t => {
                    ...
                    options += `<option value="${t.id}" data-prix="${prix}" data-libre="${t.montant_libre}">${t.nom} - ${t.montant_libre == 1 ? 'Montant Libre' : prix + ' €'}</option>`;
                });
                ...
                container.innerHTML = `
                    ...
                    <div class="w-full md:w-1/2">
                        ...
                        <div id="libre-container-${id}" class="hidden mt-2">
                            <input type="number" step="0.01" min="0" placeholder="Montant (€)" class="border p-2 rounded w-full libre-input" data-child="${id}">
                        </div>
                    </div>
        ...
        function handleTarifChange(e, childId) {
            ...
            if(isLibre) {
                ...
                selectedTarifs[childId] = { id: select.value, price: 0, isLibre: true, basePrice: 0 }; 
                ...
                selectedTarifs[childId] = { id: select.value, price: finalPrice, basePrice: priceBeforeDiscount, discount: discountPercent, isLibre: false };
                updateChildPriceDisplay(childId, finalPrice, priceBeforeDiscount, discountPercent);
            }
            calculateGrandTotal();
        }
        ...
        function calculateGrandTotal() {
            ...
            selectedChildrenIds.forEach(id => {
                ...
                if(isLibre) {
                    ...
                    if(!val || val < 0) { allValid = false; }
                    
                    const finalPrice = Math.max(0, val || 0);
                    total += finalPrice;
                    
                    selectedTarifs[id] = { id: select.value, price: finalPrice, isLibre: true, basePrice: 0 };
                    updateChildPriceDisplay(id, finalPrice);
        ...
        // --- LOGIQUE DE PAIEMENT STRIPE (handlePaymentSubmission) ---
        window.handlePaymentSubmission = async function(event) {
            event.preventDefault();
            errorMessage.classList.add('hidden');

            const totalAmount = calculateGrandTotal();
            if (totalAmount <= 0) {
                alert("Le montant total à payer est de 0.00 €.");
                return;
            }
            ...
            // 1. Préparer les données pour le backend (une seule session pour tous les enfants)
            const reservationData = {
                camp_token: token,
                camp_id: globalData.camp.id,
                inscriptions: selectedChildrenIds.map(childId => ({
                    child_id: childId,
                    tarif_id: selectedTarifs[childId].id,
                    price: selectedTarifs[childId].price, // Le prix final après remise
                })),
                total_amount: totalAmount
            };

            // 2. Appel à l'API pour créer la Session Checkout Stripe
            try {
                const res = await fetch('api/create_stripe_session_multi.php', {
                    method: 'POST',
                ...
                if(!res.ok || json.error) {
                    throw new Error(json.error || 'Erreur inconnue lors de la création de la session de paiement.');
                }
                
                // Si le camp est effectivement gratuit (cas géré par le backend), on redirige directement
                ...
                payButton.textContent = "Redirection vers Stripe...";
                const result = await stripe.redirectToCheckout({ sessionId: json.id });

                if (result.error) {
                    throw new Error(result.error.message || 'Échec de la redirection Stripe.');
                }
            } catch (e) {
                console.error("Erreur de paiement:", e);
                errorMessage.textContent = e.message || "Une erreur est survenue lors du paiement. Veuillez réessayer.";
