...
<div class="min-h-screen bg-gray-50 py-10 font-sans">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <?php if ($error): ?>
            ...
            <a href="public_infos.php" class="text-[#0A112F] font-bold hover:underline block mt-4">&larr; Retour à la sélection</a>
        <?php else: ?>
                ...
                <div class="flex flex-col md:flex-row md:items-start justify-between gap-4">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-gray-500 mt-1">Vue d'ensemble et gestion des camps pour cet organisme.</p>
                        </div>
                    </div>
                    
                    <div class="flex flex-col sm:flex-row gap-4 mt-4 md:mt-0">
                        <a href="edit_organisateur.php?id=<?= $organisateur['id'] ?>" class="inline-flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-6 rounded-xl shadow-lg transition transform hover:-translate-y-0.5">
                        ...
                        if ($solde > 0): 
                        ?>
                            <a href="demande_de_virement_info.php?organisateur_id=<?= $organisateur['id'] ?>" 
                               class="inline-flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-6 rounded-xl shadow-lg transition transform hover:-translate-y-0.5 w-full">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z"/></svg>
                                Virer l'argent (<?= number_format($solde, 2, ',', ' ') ?>€)
                            </a>
                        <?php else: ?>
                            <button disabled class="inline-flex items-center justify-center gap-2 bg-gray-500 text-white font-bold py-3 px-6 rounded-xl shadow-lg opacity-75 cursor-not-allowed">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-y-4 gap-x-8 text-sm">
                    ...
                    <?php if (!$iban_ok || !$bic_ok): ?>
                        <div class="md:col-span-3 text-red-700 text-sm mt-2 p-3 bg-red-100 rounded-lg">
                            ⚠️ Attention : IBAN ou BIC manquant. Veuillez <a href="edit_organisateur.php?id=<?= $organisateur['id'] ?>" class="font-bold underline">modifier les informations</a> pour permettre les virements.
                        </div>
                    <?php endif; ?>
            ...
            <div class="flex justify-between items-center mb-6">
                ...
                <?php if (!empty($virements)): ?>
                    <a href="pdf_export_virements_history.php?organisateur_id=<?= $organisateurId ?>" target="_blank" class="inline-flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-lg shadow-sm transition">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <a href="virement.php?t=<?= $virement['token'] ?>" class="text-[#0A112F] hover:text-blue-600">Détails</a>
                                </td>
                            </tr>
                            <?php endforeach; ?>
                        </tbody>
            ...
            <?php if (empty($camps)): ?>
                <div class="bg-white rounded-3xl shadow-sm p-12 text-center border border-gray-100">
                    ...
                    <a href="create_camp.php" class="text-[#0A112F] font-bold hover:underline">Créer un nouveau séjour &rarr;</a>
                </div>
            <?php else: ?>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    ...
                    <div class="bg-white rounded-2xl shadow-sm hover:shadow-md transition border border-gray-200 overflow-hidden flex flex-col">
                        ...
                        <div class="p-4 flex-1 flex flex-col">
                            ...
                            <a href="gestion_camp.php?t=<?= $camp['token'] ?>" class="mt-auto block w-full text-center py-2 rounded-lg bg-gray-100 text-gray-700 text-sm font-bold hover:bg-gray-200 transition">
                                Gérer les inscrits
                            </a>
                        </div>
                    </div>
